(function(){
 ...
 async function startPayment(booking){
  ...
  const c=await sb();
  const amount=booking.paymentChoice==='full'
    ? Number(booking.amountTotal)
    : Math.round(Number(booking.amountTotal)*Number(CFG.depositPercent))/100;
  const {data,error}=await c.functions.invoke('create-checkout-session',{
    body:{
  ...
  return {ok:true,checkoutUrl:data?.url||null,simulated:false};
}
 ...
 async function adminSignIn(email,password){if(CFG.mode==='demo')return {user:{email:'demo-admin@local.test'}};const c=await sb();const {data,error}=await c.auth.signInWithPassword({email,password});if(error)throw error;return data}
 ...
 async function updateStatus(id,status){if(CFG.mode==='demo'){const all=load(STORAGE),x=all.find(v=>v.id===id);if(x)x.status=status;save(STORAGE,all);return}const c=await sb();const {error}=await c.from('bookings').update({status}).eq('id',id);if(error)throw error}
...
function occupiedEnd(startMinute,duration){return startMinute+durationMinutes(duration)+(CFG.travelBufferMinutes||30)}
 ...
 window.AndeyData={slots,mins,durationMinutes,rangesOverlap,listBusy,createBooking,startPayment,markDemoPaid,adminSignIn,listAdminBookings,updateStatus,findBooking,customerCancel,customerReschedule,listBlocks,addBlock,deleteBlock,minutesToTime,priceFor,isSwim,serviceDurationOptions,occupiedEnd,resetDemo};
})();