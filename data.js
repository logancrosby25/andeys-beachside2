...
function occupiedEnd(startMinute,duration){return startMinute+durationMinutes(duration)+(CFG.travelBufferMinutes||30)}
 ...
 async function adminRescheduleBooking(id,date,startMinute,timeLabel,durationLabel){
   if(CFG.mode==='demo'){
     const all=load(STORAGE),x=all.find(v=>v.id===id);
     if(!x)throw new Error('Booking not found.');
     x.date=date;x.startMinute=startMinute;x.time=timeLabel;x.duration=durationLabel;
     x.endMinute=occupiedEnd(startMinute,durationLabel);x.status='Confirmed';
     save(STORAGE,all);return x;
   }
   const c=await sb();
   const {data,error}=await c.from('bookings').update({
     booking_date:date,
     start_minute:startMinute,
     end_minute:occupiedEnd(startMinute,durationLabel),
 ...
 window.AndeyData={slots,mins,durationMinutes,rangesOverlap,friendlyError,listBusy,adminRescheduleBooking,sendBookingNotification,createBooking,startPayment,markDemoPaid,adminSignIn,listAdminBookings,updateStatus,findBooking,customerCancel,customerReschedule,listBlocks,addBlock,deleteBlock,minutesToTime,priceFor,isSwim,serviceDurationOptions,occupiedEnd,resetDemo};
})();